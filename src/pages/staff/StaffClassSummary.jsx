...
import ReviewedSummaryView from '../../components/ReviewedSummaryView.jsx';
...
import { getReviewedSummary } from '../../utils/reviewedSummaries.js';
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        const reviewedDoc = await getReviewedSummary({
          cycleId: openCycle.id,
          classId: classMeta.id,
        });
        if (!cancelled) {
          setLiveSummary(summary);
          setReviewed(reviewedDoc);
        }
      } catch (e) {
        console.error(e);
  ...
  return (
    <div className="space-y-6">
      ...
      {!display ? (
        ...
        <>
          {/* Reviewed summary takes pride of place when present */}
          {reviewed && <ReviewedSummaryView summary={reviewed} />}
          ...
          {!reviewed && (
            <Card title="Draft summary (for human review)" icon={Bot}>
              <p className="text-stone-700">
                Below are aggregated patterns to support your reflection. They are a starting
                point, not a verdict. <strong>A management-reviewed summary will replace this section
                once one is published for this class.</strong>
              </p>
            </Card>
          )}
